...
class PendingDriver extends Component {
  constructor(props) {
    super(props);
  ...
  onRowClick(row) {
    this.props.history.push(`/pending-drivers/${row.id}`);
  }
  ...
  actionField(cell, row) {
    return (
      <>
        <Button size="sm" color="info" className="mr-2" disabled={this.state.processing} onClick={ () => this.handleActive(row.id, 1) }><i className="fa fa-bell-o"> Active</i></Button>
        <Button size="sm" color="warning" disabled={this.state.processing} onClick={ () => this.handleActive(row.id, 2) }><i className="fa fa-bell-slash-o"> Inactive</i></Button>
      </>
    )
  }
  ...
  handleActive(id, status) {
    ...
    var payload = {
      token: this.props.token,
      userid: id,
      status: status // from pending status to active/inactive status
    }
    var self = this;
    ...
    return (
      <div className="animated">
        ...
        :
          <Card>
            <CardHeader>
              <i className="icon-menu"></i>Pending Drivers{' '}
            </CardHeader>
            <CardBody>
              <BootstrapTable 
              ...
              >
                ...
                <TableHeaderColumn dataAlign="center" width='200' dataFormat={this.actionField}>Actions</TableHeaderColumn>
              </BootstrapTable>
            </CardBody>
          </Card>
        }        
