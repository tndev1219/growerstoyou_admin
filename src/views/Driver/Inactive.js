...
class InactiveDriver extends Component {
  constructor(props) {
    super(props);
  ...
  onRowClick(row) {
    this.props.history.push(`/inactive-drivers/${row.id}`);
  }
  ...
  actionField(cell, row) {
    return (
      <Button size="sm" color="info" disabled={this.state.processing} onClick={ () => this.handleActive(row.id) }><i className="fa fa-bell-o"> Active</i></Button>
    )
  }
  ...
  handleActive(id) {
    ...
    var payload = {
      token: this.props.token,
      userid: id,
      status: 1 // from inactive status to active status
    }
    var self = this;
    ...
    return (
      <div className="animated">
        ...
        :
          <Card>
            <CardHeader>
              <i className="icon-menu"></i>Inactive Drivers{' '}
            </CardHeader>
            <CardBody>
              <BootstrapTable data={this.state.driverData} version="4" bordered={false} hover pagination search options={this.options}>
